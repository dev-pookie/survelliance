...
class Config:
    ...
    W_GEOFENCE: int = 30 # Points for being in AOI
    W_LOITER: int = 25   # Points for loitering (speed near zero)
    W_UNEXPECTED: int = 15 # Points for classes NOT in typical_classes
    TYPICAL_CLASSES: List[str] = ['car', 'truck', 'bus'] # Expected traffic
...
def calculate_threat_score(is_geofence: bool, is_loitering: bool, object_class: str, confidence: float) -> int:
    ...
    # 1. Geofence Breach
    if is_geofence:
        score += Config.W_GEOFENCE
        
    # 2. Loitering
    if is_loitering:
        score += Config.W_LOITER
        
    # 3. Unexpected Class (e.g., person in a vehicle zone)
    if object_class not in Config.TYPICAL_CLASSES:
...
def analysis_page():
    ...
    if uploaded_file is not None and model:
        ...
        if st.button("▶️ Start Analysis", type="primary"):
            ...
            try:
                # RUN THE CORE PROCESSING LOGIC and store logs + trajectories
...
def reports_page():
    ...
    with tab_trajectory:
        ...
        if selected_id:
            ...
            fig.add_trace(go.Scatter(
                ...
                y=df_traj['y'], 
                mode='lines+markers', 
                name=f'ID {selected_id} Path',
                marker=dict(size=8, color=df_traj['threat_score'], colorscale='Plasma', colorbar=dict(title="Threat Score"))
            ))
    ...
    with tab_ai:
        ...
        if st.button("Run Anomaly Report"):
            with st.spinner("Analyzing frame-by-frame activities for spikes/drops..."):
    ...
    # --- FINAL DOWNLOAD SECTION (Always Visible) ---
    st.markdown("---")
    st.subheader("⬇️ Download All Reports")

    download_cols = st.columns(5)
    
    with download_cols[0]: st.download_button(label="Detailed Log (TXT)", data=st.session_state['detailed_report'], file_name=f"{Config.REPORT_NAME}_detailed_report.txt", mime="text/plain")
    with download_cols[1]: st.download_button(label="Time Logs (TXT)", data=st.session_state['time_logs'], file_name=f"{Config.REPORT_NAME}_time_logs.txt", mime="text/plain")
    with download_cols[2]: st.download_button(label="Detailed Log (JSON)", data=st.session_state['detailed_json'], file_name=f"{Config.REPORT_NAME}_detailed_report.json", mime="application/json")
    with download_cols[3]: st.download_button(label="Time Logs (JSON)", data=st.session_state['time_logs_json'], file_name=f"{Config.REPORT_NAME}_time_logs.json", mime="application/json")
